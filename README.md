...
* Adding a new record:
  ...
  VALUES 
  (650000,85000,424242,'TR-3131',CURRENT_TIMESTAMP(),'SamWhittle','Incremental test','This is a test row insertion to confirm incremental model',CURRENT_TIMESTAMP()),
  (640000,85000,424242,'TR-3131',CURRENT_TIMESTAMP(),'SamWhittle','Incremental test 2','This is a test row insertion to confirm incremental model 2',CURRENT_TIMESTAMP())
  ```
...
* Execute an SQL on the Snowflake UI to ensure the new record has been added:
  ```
  1)
  SELECT COUNT(*) FROM AIRSTATS.DEV.SILVER_AIRPORT_COMMENTS
  WHERE member_nickname = 'SamWhittle'
  2) 1 (meaning that only the record with comment_id=650000 was added,
        and the record with comment_id=640000 was ignored as it is 
        smaller than MAX(comment_id). For more context, executing the
        same select statement on RAW.AIRPORT_COMMENTS returns 2 as two
        rows were inserted into it previously.)
